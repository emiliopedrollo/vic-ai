...
<template>
  <div v-if="confirmation" class="flex align-center flex-col">
    <h2 class="font-bold text-lg mb-3">
      <span v-if="(confirmation?.details['prepare_ia_store'] || []).length > 1">Inseminações</span>
      <span v-else-if="(confirmation?.details['prepare_ia_store'] || []).length === 1">Inseminação</span>
    </h2>
    <div class="flex flex-col gap-5">
      <div v-for="attachment in (confirmation?.details['prepare_ia_store'] || [])" class="
        ...
        <div class="w-full flex flex-row gap-4 justify-space-between">
          <div>Tipo de sêmen:</div>
          <div>{{ attachment.args.semen_type }}</div>
        </div>
