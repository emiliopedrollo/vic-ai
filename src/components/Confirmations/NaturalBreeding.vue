...
<template>
  <div v-if="confirmation" class="flex align-center flex-col">
    <h2 class="font-bold text-lg mb-3">
      <span v-if="(confirmation?.details['prepare_natural_breeding_store'] || []).length > 1">Montas naturais</span>
      <span v-else-if="(confirmation?.details['prepare_natural_breeding_store'] || []).length === 1">Monta natural</span>
    </h2>
    <div class="flex flex-col gap-5">
      <div v-for="attachment in (confirmation?.details['prepare_natural_breeding_store'] || [])" class="
