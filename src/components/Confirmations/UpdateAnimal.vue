...
<template>
  <div v-if="confirmation" class="flex align-center flex-col">
    <h2 class="font-bold text-lg mb-3">
      <span v-if="(confirmation?.details['prepare_animal_update'] || []).length > 1">Atualização de animais</span>
      <span v-else-if="(confirmation?.details['prepare_animal_update'] || []).length === 1">Atualização de animal</span>
    </h2>
    <div class="flex flex-col gap-5">
      <div v-for="animal in (confirmation?.details['prepare_animal_update'] || [])" class="
        ...
        <div v-if="animal.args.earring" class="w-full flex flex-row gap-4 justify-space-between">
          <div>Brinco:</div>
          <div>{{ animal.args.earring }}</div>
        </div>
        <div v-if="animal.args.batch_slug" class="w-full flex flex-row gap-4 justify-space-between">
          <div>Lote:</div>
